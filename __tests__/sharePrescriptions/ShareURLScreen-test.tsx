...
describe("ShareURLScreen", () => {
      ...
      agent: {
        modules: {
          openId4VcHolderModule: {
            resolveSiopAuthorizationRequest:
              mockResolveSiopAuthorizationRequest,
          },
        },
      },
    });
  ...
  test("should navigate to ChoosePrescriptionsScreen when share succeeds", async () => {
    ...
    await waitFor(() => {
      expect(mockResolveSiopAuthorizationRequest).toHaveBeenCalledWith(
        "https://valid-url.com",
      );
      expect(mockSetResolvedAuthorizationRequest).toHaveBeenCalledWith(
        mockResolvedRequest,
      );
  ...
  test("should show loading state while sharing", () => {
    mockResolveSiopAuthorizationRequest.mockImplementation(
      () => new Promise(() => {}),
    );
  ...
  test("should handle error during share and clear state", async () => {
    ...
    mockResolveSiopAuthorizationRequest.mockRejectedValue(
      new Error("Invalid authorization request"),
    );
